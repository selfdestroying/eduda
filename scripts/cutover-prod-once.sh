...
#
# Заодно накатываются 24 миграции с двумя ожидаемыми остановками и разбором
...
4. Сохранить STUDENT_PW_KEY в менеджер паролей:
   ...
   Им зашифрованы пароли учеников. Ротации у ключа нет: потеряется или
   поменяется — школа перестанет видеть пароли навсегда (вход при этом
