...
- **Разговор в MAX — одна дорога и три команды.** «Начать» (`bot_started`) — приветствие с кнопкой «отправить номер»; номер пришёл — рассказ о найденных детях (`summary.ts`); **всё остальное остаётся без ответа**, включая `/start` и «привет». Это не забывчивость: повторное приветствие в ответ на реплику выглядит как сбой, а «я вас не понял» учит родителя, что писать сюда бесполезно. Исключение — команды меню `/stop`, `/resume`, `/cabinet`: их нажимают намеренно, и молчание на них было бы поломкой. Меню ставится `PATCH /me` (`ensureCommands`) один раз за запуск процесса, а не каждым проходом крона: в отличие от подписки оно не протухает.
...
- **Текст сообщения — шаблон школы** (`Organization.reminderTemplate`), а рендер и проверка живут в `@repo/core/reminder-template`. В ядре они потому, что нужны обеим половинам: планировщик собирает по ним настоящее сообщение, форма настроек — превью. Вторая копия разошлась бы молча, и школа видела бы одно, а родитель получал другое.
  ...
  - **Форматирования нет намеренно.** У MAX есть `format: markdown|html` ([NewMessageBody](https://dev.max.ru/docs-api/objects/NewMessageBody)), у VK с 5.245 — массив смещений `format_data`, а у нас в `providers/vk.ts` пока прибито `v=5.199`. Одной размеченной строкой их не накормить, поэтому шаблон — простая строка. Понадобится жирный шрифт — шаблон станет отрезками с пометками, `Sender` расширится с `text: string` до сообщения, а сериализация уедет в `providers/vk.ts` и `providers/max.ts`, где и так лежат все различия между ними.
...
- **MAX живёт за российским корневым сертификатом.** `platform-api2.max.ru` отдаёт цепочку `*.max.ru` → «Russian Trusted Sub CA» → «Russian Trusted Root CA» (Минцифры), а этого корня нет ни в CA-наборе Debian, ни во встроенном наборе Node. Без него **любой** исходящий вызов MAX падает с `TypeError: fetch failed` — на сбой сети или неверный токен это похоже, но TLS отваливается до того, как токен вообще предъявляется. VK не затронут: у него общедоверенный сертификат. Диагноз в одну строку — `curl https://platform-api2.max.ru/subscriptions` даёт `unable to get local issuer certificate`. На проде корень лежит в `apps/bots/russian_trusted_root_ca.pem` (в системный store не ставился, доверие расширено только процессу бота), а путь к нему — в `NODE_EXTRA_CA_CERTS` **записи pm2**, не в `.env`: Node читает эту переменную до разбора `--env-file`, поэтому оттуда она не работает. Задаётся один раз при заведении записи (`NODE_EXTRA_CA_CERTS=… pm2 start npm --name bots -- start`); `pm2 restart --update-env` из деплоя её не теряет, и `pm2 save` кладёт её в дамп, так что перезагрузку она тоже переживает.
