...
Прод отстаёт на 24 миграции (стоит на `20260721120000_group_name`). Порядок ниже проверен целиком на дампе прода 28.08.2026. `migrate deploy` в лоб не проходит: он останавливается дважды, и оба раза это защита миграции, а не поломка — база при остановке цела, лечится `migrate resolve`.
