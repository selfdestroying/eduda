...
pnpm --filter platform exec tsx scripts/charge-missed-makeups.ts --apply          # списать отработки, пропущенные с предупреждением
```
...
Отработки, пропущенные по старому правилу с колокольчиком, остались без цены — занятие провели, а денег за него в базе нет. Догоняет их `scripts/charge-missed-makeups.ts`: обычное списание с головного пакета, датированное днём занятия, вхолостую по умолчанию. Кошелька или свободного пакета нет — занятие остаётся ждать оплаты, как любое непокрытое.
