...
- **Дренаж по одному** с паузой 60 мс: у MAX лимит 30 rps. Отправитель выбирается по привязке (`SenderFor`): у бота школы свой токен; бота нет — школа отключила его после планирования — отказ без повторов. MAX 403/404 — не сбой доставки, а запрет сообщений: гасим привязку, иначе следующий план наберёт того же родителя снова.
