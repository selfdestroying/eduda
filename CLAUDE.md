...
Прод отстаёт на 23 миграции (стоит на `20260721120000_group_name`). Порядок ниже проверен целиком на дампе прода 28.08.2026. `migrate deploy` в лоб не проходит: он останавливается дважды, и оба раза это защита миграции, а не поломка — база при остановке цела, лечится `migrate resolve`.

На чистой БД ничего этого не нужно: все проверки проходят на пустых таблицах.

На боевом сервере весь порядок ниже выполняет `scripts/migrate-prod-once.sh` — одноразовый, с проверками до первого изменения и с восстановлением по дампу при обрыве. Ручные команды оставлены здесь, потому что по ним прогоняются дампы локально и по ним же понятно, что именно делает скрипт. Обычные деплои после перехода — `scripts/deploy.sh`.
