...
- **Рассылает школа одним ботом в каждый момент.** `ParentMessenger.ownBot`: `false` — привязка к боту ЕДУДА, `true` — к боту своей школы; уникальный ключ включает `ownBot`, потому что один аккаунт бывает подключён к обоим. Какие привязки сейчас «работают», описано один раз — `activeMessengerWhere(hasOwnBot)` в `@repo/core/messenger`; им пользуются планировщик, экран школы, кабинет родителя и карточка ученика. Привязки другого бота не удаляются: школа отключила своего бота — рассылка вернулась на привязки к ЕДУДА, подключила того же снова — заработали прежние. Родителей при смене бота никто не переносит: они заново жмут «Отправить номер» в новом боте.
