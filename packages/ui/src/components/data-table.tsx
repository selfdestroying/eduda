...
declare module '@tanstack/react-table' {
  ...
  interface ColumnMeta<TData extends RowData, TValue> {
    ...
    unit?: string
    /**
     * Границы ползунка у `range`: `[min, max]`. Без них рисуются только два поля
     * ввода — ползунку нужна шкала, а откуда её взять, знает только тот, кто
     * объявляет колонку: клиенту с серверной пагинацией видна одна страница.
     * Значения вне границ по-прежнему вводятся числом.
     */
    range?: [number, number]
    /** Шаг ползунка. По умолчанию 1. */
    step?: number
    /**
