 ...
 * рендерится один раз.
 *
 * Форматирования нет намеренно: разметка MAX (`format: markdown`) заставила бы
 * экранировать текст школы и имена учеников. Появится нужда — шаблон станет
 * отрезками с пометками, а сериализация уедет в провайдера.
 */
