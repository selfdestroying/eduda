...
export type AbsentDimensions = {
   ...
   * Предупреждённый пропуск не списывается и родителю не стоит ничего.
   */
  price: number | null
...
export function foldAbsentGroups(by: AbsentGroupBy, rows: AbsentDimensions[]): AbsentGroupRow[] {
  ...
  for (const row of rows) {
    // Тем же правилом, что и списание: непредупреждённый пропуск снимает занятие
    // с пакета, предупреждённый — нет. Поэтому деньги теряются только на них.
    const lost = row.isWarned ? 0 : (row.price ?? 0)
