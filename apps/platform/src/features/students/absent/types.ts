...
export type AbsentGroupRow = {
  ...
  students: number
  /**
   * Потеряно родителями, ₽ — цена списанных пропусков: непредупреждённых и
   * пропущенных отработок. Складывается: цена лежит на самой отметке, а не на
   * кошельке, поэтому задвоиться ей негде.
   */
  lost: number
  /**
...
export type AbsentChartPoint = {
  ...
  unwarned: number
  /**
   * Деньги, потерянные родителями за день: цена занятия там, где оно списалось.
   * Предупреждённый пропуск не списывается (`isLessonCharged`), значит и не стоит
   * родителю ничего; пропущенная отработка списывается и стоит.
   */
  lost: number
  /**
