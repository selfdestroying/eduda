  ...
  .action(async ({ ctx, parsedInput }): Promise<AbsentChartPoint[]> => {
    const rows = await prisma.attendance.findMany({
      ...
      select: {
        ...
        // пропуск предупреждённый и без отработки, либо занятие ещё ждёт оплаты.
        // И то и другое для родителя стоит нуля, поэтому выдумывать цену здесь
        // нечем и незачем.
        price: true,
    ...
    for (const row of rows) {
      ...
      if (row.isWarned) {
        ...
        // спасает посещение, а не запись. Пропущенная отработка списывается сама
        // и придёт сюда своей строкой.
      ...
      // Потеряно ровно то, что списалось: цена на строке и есть признак списания.
      // Так сюда попадает и пропущенная отработка — она платная при любом флаге.
