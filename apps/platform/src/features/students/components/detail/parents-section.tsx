...
import { cn, parentCabinetUrl } from '@/src/lib/utils'
...
function WhatsAppBadge({ phone }: { phone: string }) {
  return (
    <a
      ...
      className="bg-background text-success ring-success/30 hover:bg-success/10 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ring-1 transition-colors"
    >
...
function TelegramBadge({ phone }: { phone: string }) {
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="bg-background text-info ring-info/30 hover:bg-info/10 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ring-1 transition-colors"
    >
      <svg viewBox="0 0 24 24" className="size-3 fill-current">
...
function BotBadge({ name, connected }: { name: string; connected: boolean }) {
  return (
    <span
      className={cn(
        'bg-background inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ring-1 transition-colors',
        connected ? 'text-success ring-success/30' : 'text-muted-foreground ring-border',
      )}
    >
