...
/**
 * Ширины — в процентах: 20% забирает «Ученик», остальные 80% делят между собой
 ...
 * не «сколько хочется», а «уже нечитаемо»: у «Ученика» это обрезанное ФИО, а не
 * его комфортные 30%.
 *
 * Раньше всех упираются «Статус» и «Дата» — их доли самые маленькие. С долями
 * 11,5% и 10% бейдж «Корректировка» и «10.08.2026» перестают влезать примерно на
 * той же ширине, где стоит порог, так что интервал, в котором они подрезаны, а
 * «Ученику» ещё просторно, почти исчез.
 */
const WIDTHS = {
  student: 110,
  price: 130,
