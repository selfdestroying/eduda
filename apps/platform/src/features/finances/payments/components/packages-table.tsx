...
/**
 * Ширина числовых колонок. Суммы, числа и даты имеют известный потолок
 ...
 *
 * Порог горизонтальной прокрутки — сумма `size`: 4 × 130, 200 у «Продукта» и 150
 * у «Ученика» (столько react-table даёт колонке без явного `size`).
 */
const COLUMN_WIDTH = 130

/**
 * «Продукт» шире остальных: в нём название строки прайса, а не число с известным
 * потолком длины. Всё равно обрезается, поэтому в ячейке висит подсказка.
 */
...
function buildColumns(managerOptions: FilterOption[]): ColumnDef<PackageListItem>[] {
  return [
    ...
    {
      // Что продали. Читается со снимка на пакете, а не со связанного продукта:
      // строку прайса могли переименовать или убрать, а подпись проданного
      // обязана остаться той, по которой договаривались.
      id: 'product',
      header: 'Продукт',
      ...
      cell: ({ row }) => {
        ...
        // Пакеты, заведённые до справочника продуктов, остались без названия.
        if (!name) return '—'
        // Названия прайса длинные («36 занятий с разбивкой на 3 платежа») и в
        // ячейку не влезают, а та их обрезает: подсказка браузера — единственное
        // место, где видно целиком.
