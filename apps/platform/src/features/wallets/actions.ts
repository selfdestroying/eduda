...
export const getStudentWallets = authAction
  ...
  .action(async ({ ctx, parsedInput }) => {
    return await prisma.wallet.findMany({
      ...
      include: {
        // Свежая группа первой: предпросмотр кошелька в свёрнутом виде показывает
        // одну строку, и это должна быть та группа, куда ученика записали последней,
        // а не та, откуда он ушёл год назад. `groupId` вторым ключом — у записей,
        // заведённых одной транзакцией, `createdAt` совпадает до миллисекунды, и без
        // него порядок между ними каждый раз новый.
        studentGroups: {
          include: {
            group: { include: { course: true, location: true, schedules: true } },
          },
          orderBy: [{ createdAt: 'desc' }, { groupId: 'desc' }],
        },
