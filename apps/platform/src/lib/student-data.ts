...
// Список держим здесь, а не тянем из денежного ядра: этот модуль должен
// оставаться проверяемым в отрыве от Next. `satisfies` держит имена
// привязанными к схеме — переименование колонки сломает сборку.
