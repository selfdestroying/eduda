 ...
 *
 *   668:  28 уроков / 38 430 ₽ = семь пакетов «4 занятия» по 5 490 ₽
 *   1234: 36 уроков / 41 900 ₽ = депозит 5 000 ₽ + год 36 900 ₽
 *
 ...
 *     (17 занятий 25/26 были оценены по цене следующего года), сентябрьские
 *     занятия дешевеют с 1 372 ₽ до 1 000 ₽ — цены депозита, с которого год и
 *     начинается. Поступления по ученику уменьшаются на 17 540 ₽: 2 600 —
 *     исправление цены абонемента, 14 940 — фантомные деньги пломбы.
 *
...
async function fix(tx: Tx) {
  // ─── 1. Раздача занятий по очереди ───────────────────────────────────
  // Сначала раздача, потом пакеты: остаток каждого пакета — это его размер минус
  // розданное, и зашивать его числом нельзя. На проде за время правки успели
  // отметить ещё одно сентябрьское занятие, и зашитый остаток разошёлся с балансом.
  ...
  /** Сколько занятий легло на пакет: его остаток — размер минус это. */
  const charged = new Map<number, number>()
  for (const { packageId } of [...old.plan.values(), ...fresh.plan.values()]) {
    charged.set(packageId, (charged.get(packageId) ?? 0) + 1)
  }
  const remainingOf = (p: { id: number; lessons: number }) => p.lessons - (charged.get(p.id) ?? 0)
  ...
  await tx.package.update({
    ...
    data: {
      ...
      remaining: remainingOf(QUEUE_OLD[0]!),
    },
  })

  // Остальные шесть оплат 25/26 возвращаются в свой кошелёк и тратятся там целиком.
  for (const p of QUEUE_OLD.slice(1)) {
    await tx.package.update({
      where: { id: p.id },
      data: { walletId: WALLET_OLD, remaining: remainingOf(p) },
    })
  }

  // Депозит: пять уроков из тридцати шести, с них и начинается год.
  await tx.package.update({
    where: { id: DEPOSIT.id },
    data: {
      walletId: WALLET_NEW,
      productName: 'Депозит года 26/27',
      remaining: remainingOf(DEPOSIT),
    },
  })
  ...
  await tx.package.update({
    ...
    data: {
      walletId: WALLET_NEW,
      lessonCount: YEAR.lessons,
      price: YEAR.price,
      unitPrice: unitOf(YEAR),
      remaining: remainingOf(YEAR),
    },
  })
  await tx.payment.update({ where: { id: YEAR.id }, data: { price: YEAR.price } })

  // ─── 3. Проводки на строках посещаемости ─────────────────────────────
