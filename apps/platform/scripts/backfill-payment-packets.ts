// @ts-nocheck — скрипт домиграционной схемы, когда `Payment` был ещё и пакетом.
// Отработал один раз и остаётся здесь как след операции; под нынешний `Package` не
// переписывался и запускаться больше не должен.
/**
 ...
 * дальше повторяется та же FIFO-раздача, что делает `chargeAttendanceTx` вживую.
