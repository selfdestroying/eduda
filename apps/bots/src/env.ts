...
export const env = {
  ...
  /**
   * MAX включается только вместе с токеном: публикация бота в MAX требует
   * верифицированного юрлица РФ, и на машине разработчика токена обычно нет.
   * Обязательные переменные уронили бы на старте весь процесс.
   *
   * Без токена: роут `/max` отвечает 503, подписка не оформляется, отправлять
   * напоминания нечем.
   */
  max: maxToken
    ? {
